...
def make_merge_fanout_payload(levels, width):
    lines = [
        "n0: &n0 {%s}" % ", ".join(
...
def test_merge_key_fanout_dos_prevention(verbose=False):
    # Warm up to trigger lazy loading / imports
    yaml.safe_load("L0: &L0 { x: 1 }")

    payload = make_merge_fanout_payload(levels=6, width=8)

    run_with_timeout(payload, 1, "Merge key fan-out DoS took too long")

test_merge_key_fanout_dos_prevention.unittest = True
