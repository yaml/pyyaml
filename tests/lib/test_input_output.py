...
def test_unicode_output(unicode_filename, verbose=False):
    ...
    for allow_unicode in [False, True]:
        ...
        for encoding in [None, 'utf-8', 'utf-16-be', 'utf-16-le']:
            ...
            assert isinstance(data1, six.text_type), (type(data1), encoding)
            assert isinstance(data2, six.text_type), (type(data2), encoding)
...
def test_file_output(unicode_filename, verbose=False):
    ...
    try:
        ...
        stream = io.open(filename, 'w', encoding='utf-16-le')
        yaml.dump(data, stream, allow_unicode=True)
        stream.close()
        data3 = io.open(filename, encoding='utf-16-le').read()
        stream = open(filename, 'wb')
        yaml.dump(data, stream, encoding='utf-8', allow_unicode=True)
        stream.close()
        data4 = io.open(filename, encoding='utf-8').read()
...
def test_unicode_transfer(unicode_filename, verbose=False):
    ...
    for encoding in [None, 'utf-8', 'utf-16-be', 'utf-16-le']:
        ...
        assert isinstance(output1, six.text_type), (type(output1), encoding)
        if encoding is None:
            assert isinstance(output2, six.text_type), (type(output1), encoding)
        else:
