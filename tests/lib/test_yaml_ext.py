...
old_unsafe_dump = yaml.dump
def new_dump(data, stream=None, Dumper=yaml.CDumper, **kwds):
    return old_unsafe_dump(data, stream, Dumper, **kwds)

old_unsafe_dump_all = yaml.dump_all
def new_dump_all(documents, stream=None, Dumper=yaml.CDumper, **kwds):
    return old_unsafe_dump_all(documents, stream, Dumper, **kwds)

old_safe_dump = yaml.safe_dump
def new_safe_dump(data, stream=None, **kwds):
    return old_unsafe_dump(data, stream, Dumper=yaml.CSafeDumper, **kwds)
...
def _set_up():
    ...
    yaml.Loader = yaml.CUnsafeLoader
    yaml.BaseDumper = yaml.CBaseDumper
    yaml.SafeDumper = yaml.CSafeDumper
    yaml.UnsafeDumper = yaml.CUnsafeDumper
    yaml.Dumper = yaml.CUnsafeDumper
    yaml.scan = new_scan
    ...
    yaml.unsafe_dump = new_unsafe_dump
    yaml.unsafe_dump_all = new_unsafe_dump_all

def _tear_down():
    yaml.BaseLoader = yaml.PyBaseLoader
    yaml.SafeLoader = yaml.PySafeLoader
    yaml.UnsafeLoader = yaml.PyUnsafeLoader
    yaml.Loader = yaml.PyUnsafeLoader
    yaml.BaseDumper = yaml.PyBaseDumper
    yaml.SafeDumper = yaml.PySafeDumper
    yaml.UnsafeDumper = yaml.PyUnsafeDumper
    yaml.Dumper = yaml.PyUnsafeDumper
    yaml.scan = old_scan
    ...
    yaml.unsafe_dump_all = old_unsafe_dump_all
