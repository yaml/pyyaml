...
Function Bootstrap() {
    ...
    If(-not $(Test-Path C:\Python39)) {
        Invoke-Exe { choco.exe install python3 --version=3.9.0 -i --forcex86 --force --params="/InstallDir:C:\Python39" --no-progress }
    }

    If(-not $(Test-Path C:\Python39-x64)) {
        Invoke-Exe { choco.exe install python3 --version=3.9.0 -i --force --params="/InstallDir:C:\Python39-x64" --no-progress }
    }
