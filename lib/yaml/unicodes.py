class UnicodeCollection:
    """Attempt to map all Latex math symbol to Unicode."""
    # pylint: disable=C0103
    def __init__(self):
        self.TYPICAL_CHAR_REPLACEMENT= {}
        # Add later upcoming unicods in existing inicode dictionary
        # e.g RAW_LATEX_MATH_UNICODE or create new unicode dictionary
        # and append it to the UnicodeCollectionLis
        self.UnicodeCollectionList = [self.RAW_LATEX_MATH_UNICODE]
        self.bead_key_char()


    def bead_key_char(self):
        """Bead key char into nested dictionary to reduce searching time."""

        for collected_element in self.UnicodeCollectionList:
            for key, value in collected_element.items():
                key_dict = self.TYPICAL_CHAR_REPLACEMENT
                for key_ch in key:
                    if key_ch in key_dict:
                        key_dict = key_dict[key_ch]
                    else:
                        key_dict[key_ch] = {}
                        key_dict = key_dict[key_ch]
                key_dict["unicode"] = value


    RAW_LATEX_MATH_UNICODE = {"acute" : "\u0301",  # x́ COMBINING ACUTE ACCENT
                              ...
                              "widetilde" : "\u0303",  # x̃ COMBINING TILDE("Bbbk"        . #\U0001d55c)  ; 𝕜 MATHEMATICAL DOUBLE-STRUCK SMALL K
                              ...
                              "rbag" : "\u27c6",  # ⟆ RIGHT S-SHAPED BAG DELIMITER
                              "rbrace": "\u0007D", # } RIGHT CURLY BRACKET
                              "rbrack": "\u0005D",  # ] RIGHT SQUARE BRACKET
                              "rceil" : "\u2309",  # ⌉ RIGHT CEILING
                              "rfloor" : "\u230b",  # ⌋ RIGHT FLOOR
                              ...
                              "lbag" : "\u27c5",  # ⟅ LEFT S-SHAPED BAG DELIMITER
                              "lbrace": "\u0007b",  # { LEFT CURLY BRACKET
                              "lbrack": "\u0005b",  # [ LEFT SQUARE BRACKET
                              "lceil" : "\u2308",  # ⌈ LEFT CEILING
                              "lfloor" : "\u230a",  # ⌊ LEFT FLOOR
